define(["dojo/_base/declare", "/maps/screenshot/canReplace.js"],
		function (declare,configCanvas) 
		{
			return declare ("html2canvas",null,{
				_html2canvas : {},
				_window : null,
				_document : null,
				h2clog : function(a) {},
				constructor : function(_window,_document,staging)
				{
					/*
					...
					var _html2canvas = this._html2canvas,
					previousElement,
					computedCSS,
					html2canvas;
					this._window = _window;
					this._document = _document;
					this.h2clog = function (a) {
						if (_html2canvas.logging && _window.console && _window.console.log) {
							_window.console.log(a);
						}
					}
					_html2canvas.h2clog = this.h2clog;
					_html2canvas.Util = {};
					_html2canvas.staging = staging;
					_html2canvas._window = _window;
					_html2canvas._document = _document;
					...
					_html2canvas.Util.getCSS = function (el, attribute) {
						...
						if (_html2canvas._window.getComputedStyle) {
							if (previousElement !== el) {
								computedCSS = _html2canvas._document.defaultView.getComputedStyle(el, null);
							}
							...
							if (attribute === "backgroundPosition") {
								...
								var bPosY = el.currentStyle[attribute + "Y"];
								if(bPosY.indexOf('%')>0) bPosY=0;
								val = [toPX(attribute + "X", el.currentStyle[attribute + "X"]), toPX(attribute + "Y", bPosY)];
							} else {
					...
					_html2canvas.Util.Children = function (elem) {
						...
						try {
							...
							_html2canvas.h2clog("html2canvas.Util.Children failed with exception: " + ex.message);
							children = [];
						}
						return children;
					};
					...
					(function () {
						...
						_html2canvas.Generate.Gradient = function (src, bounds) {
							var canvas = _html2canvas.staging.createElement('canvas'),
								...
								if (gradient.type === 'linear') {
									...
									for (i = 0, len = gradient.colorStops.length; i < len; i += 1) {
										try {
											...
											_html2canvas.h2clog(['failed to add color stop: ', e, '; tried to add: ', gradient.colorStops[i], '; stop: ', i, '; in: ', src]);
										}
									}
									...
									// draw circle
									var canvasRadial = _html2canvas.staging.createElement('canvas'),
					...
					*/
					...
					_html2canvas.Parse = function (images, options) {
						_html2canvas._window.scroll(0, 0);

						var support = {
							rangeBounds: false,
							svgRendering: options.svgRendering && (function () {
								var img = new Image(),
								canvas = _html2canvas.staging.createElement("canvas");
								configCanvas(canvas,_html2canvas._window,_html2canvas._document);
								...
								_html2canvas.h2clog('html2canvas: Parse: SVG powered rendering available');
								return true;

							})()
						},
						element = ((options.elements === undefined) ? _html2canvas._document.body : options.elements[0]), // select body by default
						needReorder = false,
						numDraws = 0,
						fontData = {},
						...
						function renderFormValue(el, bounds, stack) {
							...
							for (i = 0, arrLen = cssArr.length; i < arrLen; i += 1) {
								...
								try {
									...
									_html2canvas.h2clog("html2canvas: Parse: Exception caught in renderFormValue: " + e.message);
								}
							}
							...
							if (/^(submit|reset|button|text|password)$/.test(el.type) || el.nodeName === "SELECT") {
								...
								{
									valueWrap.style.lineHeight = "normal";
									//IE has problems with anything but normal
								}
								}
						...
						function renderBackground(el, bounds, ctx) {
							...
							var background_repeat = getCSS(el, "backgroundRepeat");
							if(background_repeat)
								background_repeat = background_repeat.split(",")[0];
							else
							...
							if (typeof background_image !== "undefined" && /^(1|none)$/.test(background_image) === false) {
								...
								if (image) {
									...
									_html2canvas.h2clog("html2canvas: Error loading background:" + background_image);
									//console.log(images);
								}
							...
							switch (el.nodeName) {
								case "IMG":
									...
									if (image) {
										...
										_html2canvas.h2clog("html2canvas: Error loading <img>:" + imgSrc);
									}
									break;
								case "INPUT":
						...
						function parseElement(el, stack) {
							...
							if (getCSS(el, 'display') !== "none" && getCSS(el, 'visibility') !== "hidden"
								&& el.offsetHeight >0) { // Alternate method of checking visibility. May possibly need another workaround
						...
						*/
						...
						if (support.svgRendering) {
							...
							})(_html2canvas._document.documentElement);
						...
						stack.backgroundColor = getCSS(_html2canvas._document.documentElement, "backgroundColor");
					...
					_html2canvas.Preload = function (options) {
						...
						i,
						count = 0,
						element = options.elements[0] || _html2canvas._document.body,
						doc = element.ownerDocument || _html2canvas._document,
						...
						link.href = _html2canvas._window.location.href;
						...
						function start() {
							_html2canvas.h2clog("html2canvas: start: images: " + images.numLoaded + " / " + images.numTotal + " (failed: " + images.numFailed + ")");
							if (!images.firstRun && images.numLoaded >= images.numTotal) {
								_html2canvas.h2clog("Finished loading images: # " + images.numTotal + " (failed: " + images.numFailed + ")");
						...
						function proxyGetImage(url, img, imageObj) {
							...
							var gateway = window.location.href.substring(0, window.location.href.indexOf("esrimap"));
							scriptUrl+="&gateway="+gateway;
							//scriptUrl += 'url=' + encodeURIComponent(url) //+ '&callback=' + callback_name;
							//scriptUrl = '/ibmcognos/em4c/cgi-bin/em4c.cgi?/em4c/proxy/base64/?url=' + encodeURIComponent(url) + '&callback=' + callback_name;
							script = doc.createElement("script");

							_html2canvas._window[callback_name] = function (a) {
								//a = a.query.results.url;
								if (a.substring(0, 6) === "error:") {
									...
									/*if (imageObj.timer !== undefined) {
										// CORS succeeded
										_html2canvas._window.clearTimeout(imageObj.timer);
									}
								...
								_html2canvas._window[callback_name] = undefined; // to work with IE<9  // NOTE: that the undefined callback property-name still exists on the window object (for IE<9)
								try {
									//delete _html2canvas._window[callback_name];  // for all browser that support this
								} catch (ex) { }
							...
							try {
								...
								_html2canvas._window.document.body.appendChild(script);
								//window.setTimeout(function(){_html2canvas._window[callback_name](url);},0);
						...
						function getImages(el) {
							...
							try {
								...
								elNodeType = false;
								_html2canvas.h2clog("html2canvas: failed to access some element's nodeType - Exception: " + ex.message);
							}
							...
							if (elNodeType === 1 || elNodeType === undefined) {
								...
								try {
									...
									_html2canvas.h2clog("html2canvas: failed to get background-image - Exception: " + e.message);
								}
						...
						methods = {
							...
							cleanupDOM: function (cause) {
								...
								if (!images.cleanupDone) {
									if (cause && typeof cause === "string") {
										_html2canvas.h2clog("html2canvas: Cleanup because: " + cause);
									} else {
										_html2canvas.h2clog("html2canvas: Cleanup after timeout: " + options.timeout + " ms.");
									}
									...
									for (src in images) {
										if (images.hasOwnProperty(src)) {
											...
											if (typeof img === "object" && img.callbackname && img.succeeded === undefined) {
												...
												_html2canvas._window[img.callbackname] = undefined; // to work with IE<9  // NOTE: that the undefined callback property-name still exists on the window object (for IE<9)
												try {
													delete _html2canvas._window[img.callbackname];  // for all browser that support this
												} catch (ex) { }
												...
												images.numLoaded++;
												images.numFailed++;
												_html2canvas.h2clog("html2canvas: Cleaned up failed img: '" + src + "' Steps: " + images.numLoaded + " / " + images.numTotal);
											}
										}
									}

									// cancel any pending requests
									if (_html2canvas._window.stop !== undefined) {
										_html2canvas._window.stop();
									} else if (_html2canvas._document.execCommand !== undefined) {
										_html2canvas._document.execCommand("Stop", false);
									}
									if (_html2canvas._document.close !== undefined) {
										_html2canvas._document.close();
									}
							...
							renderingDone: function () {
								if (timeoutTimer) {
									_html2canvas._window.clearTimeout(timeoutTimer);
								}
							}
						...
						if (options.timeout > 0) {
							timeoutTimer = _html2canvas._window.setTimeout(methods.cleanupDOM, options.timeout);
						}
						_html2canvas.h2clog('html2canvas: Preload starts: finding background-images');
						images.firstRun = true;

						getImages(element);

						_html2canvas.h2clog('html2canvas: Preload: Finding images');
						// load <img> images
						...
						_html2canvas.h2clog('html2canvas: Preload: Done.');
					...
					*/
					_html2canvas.Renderer = function (parseQueue, options) {
						...
						return options._renderer._create(parseQueue, options, _html2canvas._document, queue, _html2canvas);
					...
					*/
					...
					_html2canvas.Renderer.Canvas = function (options) {
						...
						var doc = _html2canvas._document,
						...
						if (canvas.getContext) {
							_html2canvas.h2clog("html2canvas: Renderer: using canvas renderer");
							canvasReadyToDraw = true;
						} else if (options.flashcanvas !== undefined) {
							usingFlashcanvas = true;
							_html2canvas.h2clog("html2canvas: Renderer: canvas not available, using flashcanvas");
							...
							script.onload = (function (script, func) {
									...
									if (script.onreadystatechange !== undefined) {
										...
										intervalFunc = function () {
											if (script.readyState !== "loaded" && script.readyState !== "complete") {
												_html2canvas._window.setTimeout(intervalFunc, 250);
										...
										_html2canvas.h2clog("html2canvas: Renderer: Can't track when flashcanvas is loaded");
									}
								...
								if (typeof _html2canvas._window.FlashCanvas !== "undefined") {
									_html2canvas.h2clog("html2canvas: Renderer: Flashcanvas initialized");
									_html2canvas._window.FlashCanvas.initElement(canvas);
						...
						methods = {
							_create: function (zStack, options, doc, queue, _html2canvas) {
								...
								newCanvas,
								bounds,
								testCanvas = _html2canvas.staging.createElement("canvas"),
								...
								_html2canvas.h2clog("html2canvas: Renderer: Canvas renderer done - returning canvas obj");
				...
				loadOptsAndRun: function (elements, opts) {
					...
					log = this.h2clog; // for renderers
					Renderer = {
						Canvas: undefined // We are assuming this will be used
					};
		            // for pages without images, we still want this to be async, i.e. return methods before executing
					_html2canvas._window.setTimeout(function () {
